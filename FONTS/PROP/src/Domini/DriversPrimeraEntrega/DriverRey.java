...
public class DriverRey {
    ...
    public static Rey iniPieza(boolean esNegra, String[] tmp) {
        Rey r = new Rey(esNegra, Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]));
        eliminarDuplicados(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]));
        if(esNegra) {
            PiezasNegras.add(r);
            estadoTablero[Integer.parseInt(tmp[0])][Integer.parseInt(tmp[1])] = 'k';
        }
        else  {
            PiezasBlancas.add(r);
            estadoTablero[Integer.parseInt(tmp[0])][Integer.parseInt(tmp[1])] = 'K';
    ...
    private static void printMenuPrincipal() {
        System.out.println("Bienvenido al Driver de Rey. Selecciona qué deseas testear:");
        System.out.println("    1- Alta objeto Pieza Rey");
        System.out.println("    2- Estado del tablero (para verificar que las piezas estén correctamente colocadas en el)");
        System.out.println("    3- Verificar función esMovimientoOK de la clase Rey");
        System.out.println("    4- Verificar función movimientosPosibles de la clase Rey");
                ...
                case 1:
                    ...
                    while(!inputOK) {
                        ...
                        if(verificarEntrada(tmp)) {
                            inputOK = true;
                            Rey r = iniPieza(esNegraInput, tmp);
