...
public class DriverReina {
    ...
    public static Reina iniPieza(boolean esNegra,  String[] tmp) {
        Reina r = new Reina(esNegra, Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]));
        eliminarDuplicados(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]));
        if(esNegra) {
            PiezasNegras.add(r);
            estadoTablero[Integer.parseInt(tmp[0])][Integer.parseInt(tmp[1])] = 'q';
        }
        else  {
            PiezasBlancas.add(r);
            estadoTablero[Integer.parseInt(tmp[0])][Integer.parseInt(tmp[1])] = 'Q';
    ...
    private static void printMenuPrincipal() {
        System.out.println("Bienvenido al Driver de Reina. Selecciona qué deseas testear:");
            ...
            switch(op) {
                ...
                case 1:
                    ...
                    while(!inputOK) {
                        ...
                        if(verificarEntrada(tmp)) {
                            inputOK = true;
                            Reina r = iniPieza(esNegraInput, tmp);
                ...
                case 4:
                    ...
                    if(Character.isUpperCase(estadoTablero[posX][posY])) { //pieza blanca
                        for(int i = 0; i < PiezasBlancas.size(); ++i) {
                            if(PiezasBlancas.get(i).getTipo() == 'Q' && PiezasBlancas.get(i).getPosX() == posX && PiezasBlancas.get(i).getPosY() == posY) test2 = PiezasBlancas.get(i);
                        }
                    }
                    else { //Pieza negra
                        for(int i = 0; i < PiezasNegras.size(); ++i) {
                            if(PiezasNegras.get(i).getTipo() == 'q' && PiezasNegras.get(i).getPosX() == posX && PiezasNegras.get(i).getPosY() == posY) test2 = PiezasNegras.get(i);
                        }
                    }
