...
public class DriverTorre {
    ...
    public static Torre iniPieza(boolean esNegra,  String[] tmp) {
        Torre t = new Torre(esNegra, Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]));
        eliminarDuplicados(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]));
        if(esNegra) {
            PiezasNegras.add(t);
            estadoTablero[Integer.parseInt(tmp[0])][Integer.parseInt(tmp[1])] = 'r';
        }
        else  {
            PiezasBlancas.add(t);
            estadoTablero[Integer.parseInt(tmp[0])][Integer.parseInt(tmp[1])] = 'R';
    ...
    private static void printMenuPrincipal() {
        ...
        System.out.println("    4- Verificar función movimientosPosibles de la clase Torre");
    ...
    public static void main(String[] args) {
        ...
        while(driverIsRunning) {
            ...
            switch(op) {
                /*
                    Opciones:
                    1. Alta objeto Reina, situado en el tablero
                ...
                case 1:
                    ...
                    while(!inputOK) {
                        ...
                        if(verificarEntrada(tmp)) {
                            inputOK = true;
                            Torre t = iniPieza(esNegraInput, tmp);
                            System.out.println("Objeto Torre creado con exito.");
                        }
                    }
                    break;
                case 2:
                ...
                case 3:
                    ...
                    if(Character.isUpperCase(estadoTablero[Integer.parseInt(aux[0])][Integer.parseInt(aux[1])])) { //pieza blanca
                        for(int i = 0; i < PiezasBlancas.size(); ++i) {
                            if(PiezasBlancas.get(i).getTipo() == 'R') test = PiezasBlancas.get(i);
                        }
                    }
                    else { //Pieza negra
                        for(int i = 0; i < PiezasNegras.size(); ++i) {
                            if(PiezasNegras.get(i).getTipo() == 'r') test = PiezasNegras.get(i);
                        }
                    }
                ...
                case 4:
                    ...
                    if(Character.isUpperCase(estadoTablero[posX][posY])) { //pieza blanca
                        for(int i = 0; i < PiezasBlancas.size(); ++i) {
                            if(PiezasBlancas.get(i).getTipo() == 'R' && PiezasBlancas.get(i).getPosX() == posX && PiezasBlancas.get(i).getPosY() == posY) test2 = PiezasBlancas.get(i);
                        }
                    }
                    else { //Pieza negra
                        for(int i = 0; i < PiezasNegras.size(); ++i) {
                            if(PiezasNegras.get(i).getTipo() == 'r' && PiezasNegras.get(i).getPosX() == posX && PiezasNegras.get(i).getPosY() == posY) test2 = PiezasNegras.get(i);
                        }
                    }
